...
import { getProjectId, parentOrgId } from "@/lib/jwt";
...
const TemplateItem: React.FC<TemplateItemProps> = ({
  ...
  const fetchUsage = async () => {
    try {
      const response = await axios.post("/get_template_usage_percentage", {
        p_template_id: Number(template_id),
      ...
      if (response.data.issuccessful) {
        const usage =
          response.data.data?.templates?.[0]
            ?.get_template_usage_percentage ?? null;

        setUsagePercentage(usage ? parseFloat(usage) : 0);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  async function copyToStaging(p_project_id: number, p_template_id: number) {
    try {
      ...
      console.error("Error copying template to staging area:", error);
    }
  }
  return (
    <div
      ...
      onClick={async () => {
        ...
        router.push(
          `/create-project/templates/${template_id}/preview?name=${encodeURIComponent(template_name)}`
        )
      }
      }
    >
      <div className="p-5 flex flex-col flex-1">
        <div className="flex justify-between items-start gap-3 mb-2">
          <div className="flex flex-col gap-2 min-w-0">
            <span className={`w-fit inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium uppercase tracking-wide border ${badgeStyles}`}>
              {type}
            </span>
            <h3 className="font-semibold text-gray-900 text-base leading-snug truncate pr-2" title={template_name}>
              {template_name}
            </h3>
          </div>
          <button
        ...
        <div className="mt-auto pt-4 border-t border-gray-100">
          <div className="flex items-center justify-between text-xs text-gray-400 mb-3">
            <span className="truncate max-w-[120px]" title={creator_name}>{creator_name}</span>
